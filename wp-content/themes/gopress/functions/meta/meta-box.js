jQuery(document).ready(function($) {
	...
	// colorpicker field
	$('.gopress-color-picker').each(function(){
		var $this = $(this),
			id = $this.attr('rel');

		$this.farbtastic('#' + id);
	});
	$('.gopress-color-select').click(function(){
		$parent = $(this).parent();
		$(this).siblings('.gopress-color-picker').toggle();
		$('.gopress-color-select', $parent).toggle();
		return false;
	});

	// add more file
	$('.gopress-add-file').click(function(){
	...
	// delete file
	$('.gopress-upload').delegate('.gopress-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
	...
	$('.gopress-images').each(function(){
		...
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.gopress-images-data').val();
	...
	// thickbox upload
	$('.gopress-upload-button').click(function(){
	...
	// thickbox upload
    window.formfield = '';
    $('.gopress-upload-single-image').live('click', function() {
        window.formfield = $('.gopress-single-image',$(this).parent());
